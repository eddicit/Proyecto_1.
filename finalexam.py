...
def mostrar_menu():
    """
    Muestra el menú principal del Diccionario Informático con opciones
    para buscar un término, ver información acerca del diccionario, o salir.
    """
    print("\nBienvenido al Diccionario Informático de eddy el inge") #se muestra en pantalla el mensaje de bienvenida
    print("1. Buscar el término") #se muestra en pantalla el menseje para buscar el termino
    print("2. Acerca de") #se muestra en pantalla el mensaje acerca del termino
    print("3. Salir") #se muestra en pantalla el mensaje para salir

def buscar_termino():
    """
    Permite al usuario buscar un término en el diccionario. Si el término
    se encuentra, se muestra su definición. El usuario puede continuar
    buscando términos hasta que elija salir.
    """
    while True:
        termino = input("\nIngrese el término que desea buscar (o 'salir' para regresar al menú): ").strip().lower() #se ingresa el termino que queremos buscar
        
        if termino == 'salir':
            break

        if termino in diccionario_informatico:
            print(f"{termino.capitalize()}: {diccionario_informatico[termino]}\n") #se muestra en pantalla el termino que se desea buscar 
        else:
            print("Término no encontrado. Intente nuevamente.\n") #se muestra en pantalla el termino no encontrado

def acerca_de():
    """
    Muestra información acerca del Diccionario Informático, incluyendo
    su propósito y el creador.
    """
    print("\nDiccionario Informático del inge Eddy") #se muestra en pantalla el mensaje del diccionario 
    print("Este diccionario fue desarrollado por el inge Eddy que sirve para entender términos básicos de informática.") #se muestra en pantalla quien lo desarrollo y el funcionamiento

while True:
    mostrar_menu()
    opcion = input("\nSeleccione una opción: ").strip() #se selecciona una opcion
    ...
    if opcion == '1':
        ...
        print("Gracias por usar el Diccionario Informático del inge Eddy. ¡Hasta pronto!") #se muestra en pantalla el agradecimiento
        break
    else:
        print("Opción no válida. Por favor, seleccione una opción válida.") #se muestra en pantalla el mensaje la opcion no valida
